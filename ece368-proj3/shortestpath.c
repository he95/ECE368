...
void findpath(long int temp_start,long int temp_dest,struct node node_array[],long int node_size);
void dijkstra(long int temp_start,struct node node_array[],long int node_size,long int Q_array[], long int dest);
long int findmin(struct node node_array[], long int Q_array[],long int node_size);
int main(int argc, char** argv){
  ...
  while(i < nodes){
       ...
       sscanf(line,"%ld %ld %ld",&temp, &(temp_node.x),&(temp_node.y));
...
void findpath(long int temp_start,long int temp_dest,struct node node_array[],long int node_size){
  long int i;
  long int Q_array[node_size];
  for (i=0;i<node_size;i++)
  {
    if(i != temp_start)
    {
      node_array[i].distance = DBL_MAX;
...
void dijkstra(long int temp_start,struct node node_array[],long int node_size,long int Q_array[],long int dest)
{
  long int pathnode[1000];

  long int temp_size = node_size;
  while(temp_size != 0){
  long int u = findmin(node_array, Q_array,node_size);
  Q_array[u] = -1;
